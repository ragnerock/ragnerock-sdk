...
If `commit()` fails partway, earlier successful ops stay applied. The exception tells you exactly what made it through:
...
`__exit__` does **not** auto-commit. If the block exits with staged ops — including via exception — they're discarded.
...
To paginate through a listing of objects, use the following:
